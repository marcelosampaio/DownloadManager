...
class PersistenceManager {
    ...
    private let appDomain = "Download Manager"
    ...
    public func addFile(id: String, data: NSData) -> (absolutePath: String?, error: NSError?) {
        ...
        do {
            ...
            if (dataSize > deviceRemainingFreeSpaceInBytes()) {
                let errorDomain = appDomain
                let errorCode = 8000
                let errorUserInfo = [NSLocalizedDescriptionKey : "Erro por falta de espaço"]
            ...
            let errorDomain = appDomain
            let errorCode = 8000
            let errorUserInfo = [NSLocalizedDescriptionKey : "Erro ao salvar arquivo"]
            let errorObject = NSError.init(domain: errorDomain, code: errorCode, userInfo: errorUserInfo)
